...
function blankStats(player){return {player,team:player.team||'',teams:player.teams||[],positionId:player.positionId||'',totalActions:0,appearances:0,minutes:0,ballInPlayMins:0,positiveActions:0,negativeActions:0,carry:0,carryMetres:0,postContactMetres:0,carryDominant:0,carryContact:0,tackleAttempts:0,tackleMade:0,tackleDominant:0,colours:{Gold:0,Green:0,Yellow:0,Red:0},reviewTotal:0,typeByColour:{Gold:{},Green:{},Yellow:{},Red:{}},timeByColour:{Gold:{},Green:{},Yellow:{},Red:{}},typeCounts:{},positiveActionTypes:{},negativeActionTypes:{},shirt:player.shirt||'',position:player.position||''};}
function ensurePlayer(match, players, name, team, data={}){
  ...
  if(data.shirt&&!p.shirt) p.shirt=String(data.shirt).padStart(2,'0'); if(data.position&&!p.position) p.position=data.position; if(data.positionId&&!p.positionId) p.positionId=data.positionId;
  let st=match.playerStats.get(key); if(!st){ st=blankStats(p); match.playerStats.set(key,st); }
  if(data.shirt&&!st.shirt) st.shirt=String(data.shirt).padStart(2,'0'); if(data.position&&!st.position) st.position=data.position; if(data.positionId&&!st.positionId) st.positionId=data.positionId; st.team=p.team||team; st.teams=p.teams||[team].filter(Boolean);
...
function buildMatch(item,biRows,reviewRows,xmlText,players){
  ...
  const applyXmlMinutes=(st,info)=>{ if(!info) return false; if(info.minutes>0) st.minutes=info.minutes; if(info.ballInPlayMins>0) st.ballInPlayMins=info.ballInPlayMins; if(info.shirt&&!st.shirt) st.shirt=String(info.shirt).padStart(2,'0'); if(info.position&&!st.position) st.position=info.position; if((info.positionId||info.position)&&!st.positionId) st.positionId=info.positionId||info.position; return true; };
