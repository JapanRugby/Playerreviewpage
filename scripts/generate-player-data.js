...
function teamBaseName(team){ const parts=String(team||'').split('/').map(x=>x.trim()).filter(Boolean); return parts[0]||String(team||'').trim()||'Unknown'; }
...
function ensureTeamStats(match,team){ const name=teamBaseName(team); const map=getTeamStatsMap(match); if(!map.has(name)) map.set(name,emptyTeamStats(name)); return map.get(name); }
function rowNormText(row){return [row.actionName,row.ActionTypeName,row.ActionResultName,row.qualifier3Name,row.qualifier4Name,row.qualifier5Name,row.qualifier6Name,row.qualifier7Name].map(x=>String(x||'')).join(' | ').toLowerCase();}
function isLineoutWin(row){ const res=norm(row.ActionResultName); return /won/.test(res) && !/lost/.test(res); }
function isScrumWin(row){ const res=norm(row.ActionResultName); const type=norm(row.ActionTypeName); return /won|retained|complete|success/.test(res+' '+type) && !/lost|fail/.test(res+' '+type); }
function isTurnoverConcededRow(row){ const txt=rowNormText(row); const action=norm(row.actionName); const res=norm(row.ActionResultName); if(txt.includes('turnover conceded')||txt.includes('turnover lost')) return true; if(action.includes('lineout') && (/lost|opp/.test(res))) return true; if(action.includes('scrum') && (/lost|opp/.test(res))) return true; return false; }
function trackTeamMatchStats(match,row,st){
  const team=String(row.teamName||st?.team||st?.player?.team||'').trim(); if(!team) return; const t=ensureTeamStats(match,team);
  const action=String(row.actionName||'').trim(); const actionN=norm(action); const txt=rowNormText(row);
  if(action==='Carry'){
    const metres=toNumber(row.Metres2), pcm=toNumber(row.Metres3); t.carryMetres+=metres; const pid=parseInt(String(st?.positionId||row.playerpositionId||row.PositionID||row.PosID||'').match(/\d+/)?.[0]||'0',10); const grp=(pid>=1&&pid<=8)?'fwds':'bks'; t.carry[grp].carries++; t.carry[grp].metres+=metres; t.carry[grp].pcm+=pcm;
  }
  if(action==='Kick') t.kickingMetres+=Math.max(toNumber(row.Metres),toNumber(row.Metres2));
  ...
  const x=toNumber(row.x_coord); const start=toNumber(row.ps_timestamp), end=toNumber(row.ps_endstamp); if(x>=78 && end>start) t.opp22Time+=(end-start)/60;
}
